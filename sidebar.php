...
<div class="widget wid-categories">
   ...
   $layidrandom="SELECT ID,Ten  from theloai" ;
...
?>
	<li><a href="kind.php?Matheloai=<?php echo $d["ID"] ?>"><?php echo $d["Ten"] ?></a></li>
	<?php
	}
}
	?>
...
<div class="widget wid-categories">
   ...
   require 'inc/myconnect.php';
   //lay san pham theo id
   $layid="SELECT ID,Ten  from nhaxuatban" ;
   $kq = $conn->query($layid);
   if ($kq ->num_rows > 0) {
	// output data of each row
	while($d = $kq ->fetch_assoc()) {
...
?>
	<li><a href="nxb.php?manhasx=<?php echo $d["ID"] ?>"><?php echo $d["Ten"] ?></a></li>
	<?php
	}
}
	?>
...
<div class="widget wid-product">
	<div class="heading"><h4>Sách mới nhất</h4></div>
						...
						<?php
require 'inc/myconnect.php';
$query="SELECT * from sanpham ORDER BY date DESC limit 3;";
$rs = $conn->query($query);
if ($rs->num_rows > 0) {
   // output data of each row
